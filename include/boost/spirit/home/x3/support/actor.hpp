...
//        {}
    ...
    struct local_eval
    {
        template <typename Env>
        auto eval(Env const& env) const->
            decltype(std::get<N>(x3::get<locals_tag>(fusion::at_c<0>(env.args()))))
        {
            return std::get<N>(x3::get<locals_tag>(fusion::at_c<0>(env.args())));
        }
    };
}}}}
...
namespace boost { namespace spirit { namespace x3
{
    ...
    template <int N>
    using local_actor = make_actor<detail::local_eval<N>>;
