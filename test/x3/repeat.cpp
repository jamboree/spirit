...
auto val(T n)
{
    return [n](boost::spirit::x3::unused_type) { return n; };
}
...
main()
{
    ...
    { // actions
        std::string v;
        BOOST_TEST(test("bbbb", repeat(4)[char_] / [&v](std::string const& attr) { v = attr; }) && 4 == v.size() &&
            v[0] == 'b' && v[1] == 'b' && v[2] == 'b' &&  v[3] == 'b');
    }
    { // more actions
        std::vector<int> v;
        BOOST_TEST(test("123 456 789", repeat(3)[int_] / [&v](std::vector<int> const& attr) { v = attr; }, space) && 3 == v.size() &&
